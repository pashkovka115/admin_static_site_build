...
abstract class Node
{
     ...
     *
     * @param \Lib\DiDom\Node|DOMNode|array $nodes The prepended child
     *
     ...
     *
     * @param \Lib\DiDom\Node|DOMNode|array $nodes The appended child
     *
     * @return Element|Element[]
     *
     * @throws LogicException if the current node has no owner document
    ...
    public function appendChild($nodes)
    {
        ...
        foreach ($nodes as $node) {
            if ($node instanceof \Lib\DiDom\Node) {
                $node = $node->getNode();
            }
     ...
     *
     * @param \Lib\DiDom\Node|DOMNode $node The new node
    ...
    public function insertAfter($node, $referenceNode = null): self
    {
        ...
        if ($referenceNode instanceof \Lib\DiDom\Node) {
            $referenceNode = $referenceNode->getNode();
        }
     ...
     * Replaces a child.
     *
     * @param \Lib\DiDom\Node|DOMNode $newNode The new node
    ...
    public function replace($newNode, bool $clone = true): Element
    {
        ...
        if ($newNode instanceof \Lib\DiDom\Node) {
