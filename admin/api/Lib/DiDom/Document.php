...
use Lib\DiDom\DocumentFragment;
use Lib\DiDom\Element;
use Lib\DiDom\Encoder;
use Lib\DiDom\Errors;
use Lib\DiDom\Exceptions\InvalidSelectorException;
use Lib\DiDom\Query;
use DOMAttr;
use DOMCdataSection;
use DOMComment;
...
class Document
{
     ...
     */
    public function __construct($string = null, bool $isFile = false, string $encoding = 'UTF-8', string $type = \Lib\DiDom\Document::TYPE_HTML)
    {
     ...
     * @param string $type The document type
     *
     * @return \Lib\DiDom\Document
     */
    public static function create($string = null, bool $isFile = false, string $encoding = 'UTF-8', string $type = \Lib\DiDom\Document::TYPE_HTML)
    {
        return new \Lib\DiDom\Document($string, $isFile, $encoding, $type);
    }
     ...
     */
    public function load(string $string, bool $isFile = false, string $type = \Lib\DiDom\Document::TYPE_HTML, int $options = null): void
    {
        if ( ! in_array(strtolower($type), [\Lib\DiDom\Document::TYPE_HTML, \Lib\DiDom\Document::TYPE_XML], true)) {
        ...
        if (strtolower($type) === \Lib\DiDom\Document::TYPE_HTML) {
        ...
        if ($this->type === \Lib\DiDom\Document::TYPE_HTML) {
    ...
    {
        $this->load($html, false, \Lib\DiDom\Document::TYPE_HTML, $options);
    }
    ...
    {
        $this->load($filename, true, \Lib\DiDom\Document::TYPE_HTML, $options);
    }
    ...
    {
        $this->load($xml, false, \Lib\DiDom\Document::TYPE_XML, $options);
    }
    ...
    {
        $this->load($filename, true, \Lib\DiDom\Document::TYPE_XML, $options);
    }
     ...
     *
     * @param \Lib\DiDom\Document|DOMDocument $document The compared document
     *
     * @return bool
     *
    ...
    {
        if ($document instanceof \Lib\DiDom\Document) {
    ...
    {
        return $this->type === \Lib\DiDom\Document::TYPE_HTML ? $this->html() : $this->xml();
    }
}
